...
- `markdown.rs` — pulldown-cmark event loop (using `into_offset_iter()`) producing styled, word-wrapped lines. Handles headings, code blocks, blockquotes, lists (ordered/unordered), task lists, inline formatting, horizontal rules, and Obsidian-style `[[wikilinks]]` (via `ENABLE_WIKILINKS`). Each `StyledLine` carries an optional `source_line` for task items, enabling checkbox toggling.
...
**Key dependencies**: ratatui 0.30, crossterm 0.28, pulldown-cmark 0.13, notify 7, unicode-width 0.2, serde 1, toml 0.8. Version matters — ratatui and crossterm APIs change significantly between major versions.
